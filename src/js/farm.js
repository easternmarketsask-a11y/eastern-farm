  ...
  const farm = {
    ...
    openPlotCare(plotIdx, plot, def) {
      ...
      if (waterBtn) waterBtn.onclick = () => {
        Farm.ui.hideModal();
        if (Farm.farmer && Farm.farmer.enqueueWaterAll) { Farm.farmer.enqueueWaterAll(plotIdx); return; }
    ...
    harvestPlot(plotIdx, evt) {
      ...
      if (Farm.warehouse && Farm.warehouse.refreshBadge) Farm.warehouse.refreshBadge();
      // New crops in the barn may make an order fillable → refresh the board badge

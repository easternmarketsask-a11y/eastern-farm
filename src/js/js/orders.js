  ...
  const orders = {
    ...
    fulfill(orderId) {
      ...
      /* 🔒 totalDeliveries 从大宗卖货搬到这里（2026-08-22 关掉无限收购）。
         它原来只在 state.deliverWarehouse 自增，直接删掉那条路会让它**永久冻结**，
         连带全部静默失效：life-story 的 deliver_first / deliver 25 / 100 章节、
         排行榜与邻居卡片的交付数、daily.js 的 isNewbie(<3)、漏斗 sell_first、
         以及新手引导第三步的推进判据。语义上这里也更对 —— 它本来的注释就是
         「warehouse → Eastern Market deliveries」，交订单正是这件事。 */
      ...
      /* 每日首单 +20%：本来挂在一键卖货上，那是玩家每天回来的钩子之一，
         不该跟着无限收购一起消失 —— 改挂在「今天第一次交单」上。 */
      let firstBonus = 0;
      if (!d.dailyClaims.firstDeliveryDone) {
        firstBonus = Math.round(order.coins * 0.2);
        if (firstBonus > 0) Farm.state.addCoins(firstBonus);

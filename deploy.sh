...
if ! node scripts/verify/car-motion-test.mjs; then
  echo "—— 部署中止(开车动画契约)"
  exit 1
fi
...
if ! node scripts/verify/feedback-reply-test.mjs; then
  echo "—— 部署中止：反馈自动回信契约没过"
  exit 1
fi
...
# 闸门 O: 「怎么玩」内容红线(约 1 秒)
# 2026-08-24 加。guide.js 是**没人会主动去读的文件** —— 订单制上线、大宗收购
# 删掉两天之后,「怎么玩」头两条还在教「把谷仓的菜卖给东方超市」,教的是一个
# 已经不存在的按钮。它不抛异常、不报错,冒烟测试照样绿,是 Chris 让优化时才发现的。
# 以后删/改任何玩法,先在这个测试里加一条断言,再去改 guide.js。
echo "▶ 闸门 O: 「怎么玩」内容红线(约 1 秒)…"
if ! node scripts/verify/guide-content-test.mjs; then
  echo "—— 部署中止：「怎么玩」在教已经不存在的玩法,或漏了必须说的规矩。"
  exit 1
fi

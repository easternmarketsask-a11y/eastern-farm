...
- [x] 批 3 首屏构图（`33a27c4`）
- [x] deploy.sh + 生产实测（`ef-2608111035`）

## 上线实测（生产站 farm.easternmarket.ca，2026-08-11）

三批合并部署为 `ef-2608111035`。生产 CDP 实测（390×844）：

- **零 console 报错 / 零未捕获异常**
- 世界杯：`worldcup` 相关请求 0 条、`Farm.worldcup` 不存在、开屏与农场入口均无
- 渲染器：`mapview.js` 线上返回 **404**（已删）、`Farm.mapView` undefined、
  `farmStyle()` 恒 `'iso'`、老值 `topdown`/`classic` 均收敛且**存档字段原样保留**
- 指南：画风按钮 0 个、走动小动物开关仍在
- 构图：zoom 1.548 / 地块屏宽 82px / 地块块占屏宽 73.5%，与本地一致
- `script` 标签 52 → **50**
- 进场总下载 **1.08MB → 1.00MB**（js 302→288KB，worldcup 那 58KB 整组消失）
- 回归确认：`worldcup.html` 200、`/redeem/` 200（两个页面照旧可用）

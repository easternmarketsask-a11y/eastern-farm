...
- 微信分享卡改用宣传油画底 + 东方农场（不再是简笔画快乐农场）。
- 未单独重绘的作物：生长期不再用 🌱 emoji，走 crop-art 矢量/插画，坐进垄沟田。
- 宣传文案/主站 snippet 里的「快乐农场」改成东方农场。

### 默认不画乡路（2026-08-19）

Chris：「农场里那条土路看着奇怪，还是默认不要吧」。
`SHOW_COUNTRY_ROAD=false`：程序化乡路、沿路行人、路口指路牌都不画；路心不再挡建筑/水塘/菜地。建造里玩家自己刷的「小路」仍在。老存档坐标不动。`true` 可恢复。

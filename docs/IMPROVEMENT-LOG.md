...
**多数质量维度已 🟢**。剩余候选多为低优先或需实机/业务判断：

- [ ] **i18n 一致性（低优先）**：英文建筑标签丢功能提示（`小屋·种子店`→`Cottage`、
      `谷仓·仓库`→`Barn`）。中文点明用途，英文没有。
- [ ] **维度 9（音频）**：音效齐全度 / 音量协调——需实机听，无头难验。
- [ ] **维度 8（取景）**：农场默认偏空旷——业主刻意调过，需 Chris 决策。
- [ ] **节日深度（V2）**：data/events.json 的端午/清明/重阳/冬至接线——需先补节日
      作物/任务/装饰，属 V2 功能，需 Chris 决策，不在 polish 范围。

> 已接近「无明显改善空间」。后续每轮若无真问题，将如实记录「本轮未发现可改项」
> 而非制造琐碎改动；并放慢节奏。

...
- [ ] **live 节日 2029 窗口**：events.js 的春节/中秋止于 2028，2029 需补正确农历日期
      （春节 2029-02-13、中秋 2029-09-22）——3 年后的内容悬崖，低优先但 live。
- [ ] **i18n 一致性（低优先）**：英文建筑标签丢功能提示（`小屋·种子店`→`Cottage`）。
- [ ] **维度 9（音频）**：音效齐全度（需实机听，难无头验证）。
- [ ] 进度盘点：7 轮后多数维度 🟢，且本轮抓到一个真 bug（春节日期）。仍有 live 2029
      窗口等少量实项；接近「无明显改善空间」，后续可放慢节奏。

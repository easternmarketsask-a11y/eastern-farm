...
const SIZES = [
  ...
  { name: 'hero-portrait',  w: 540,  h: 960,  dsf: 2, zoom: 1.42, dy: -70,  dx: -80 },
  { name: 'hero-square',    w: 600,  h: 600,  dsf: 2, zoom: 1.30, dy: -20,  dx: -60 },
  { name: 'hero-landscape', w: 960,  h: 540,  dsf: 2, zoom: 1.55, dy: -10,  dx: -50 },
  { name: 'hero-wide',      w: 1200, h: 500,  dsf: 2, zoom: 1.45, dy: 0,    dx: -40 },
];
...
const SHOWCASE = `(async () => {
  ...
  const CROPS = ['shanghai_miao','hu_luo_bo','eggplant','bo_cai','niu_jiao_jiao',
                 'xiao_cong','da_bai_cai','cai_xin','xi_lan_hua','bai_luo_bo',
                 'jiucai','tong_hao','wa_wa_cai','you_mai_cai','cilantro',
                 'dong_gua','wo_sun','ji_mao_cai','suan_tai','tomato'];
  const OX = 6, OY = 5, W = 5;
  ...
  // ---- 建筑：围着菜地摆一圈，留出前景 ----
  d.map = [
    { type: 'home', gx: 3, gy: 4, lv: 5 },      // 满级的家：彩旗 + 金色灯串
    { type: 'coop', gx: 3, gy: 7 },
    { type: 'barn', gx: 12, gy: 5 },
    { type: 'greenhouse', gx: 12, gy: 8 },
    { type: 'house', gx: 7, gy: 10 },           // 菜摊（招牌写「菜摊」）—— 乡路会自动从摊前过
    { type: 'well', gx: 5, gy: 3 },
    { type: 'wheel', gx: 11, gy: 11 },
    // 灯笼当路灯用，分立菜摊两侧；别插在菜地中间（上一版像凭空立的黑杆子）
    { type: 'lantern', gx: 6, gy: 12 },
    { type: 'lantern', gx: 10, gy: 12 },
    // 后景树排做纵深，前景两棵压边角
    { type: 'tree', gx: 1, gy: 2 }, { type: 'tree', gx: 5, gy: 1 },
    { type: 'tree', gx: 10, gy: 1 }, { type: 'tree', gx: 14, gy: 2 },
    { type: 'tree', gx: 15, gy: 6 }, { type: 'tree', gx: 2, gy: 11 },
    { type: 'bush', gx: 11, gy: 4 }, { type: 'bush', gx: 5, gy: 9 },
    { type: 'bush', gx: 13, gy: 10 },
  ];

  // ---- 地形：水塘 + 一条小路 ----
  d.mapTerrain = {};
  for (let x = 13; x <= 15; x++) for (let y = 12; y <= 14; y++) d.mapTerrain[x + ',' + y] = 'water';
  d.mapTerrain['12,13'] = 'water'; d.mapTerrain['16,13'] = 'water';
  ...
  const spots = [[9,9],[5,5],[6,11],[13,12]];   // 鸡在地头、猫在井边、狗在摊前、鸭在塘里
